...
int main() {
	...
	cout << "demo:how to find a key in a Chain List,input what you want to find" << endl;
	cin >> key;
	node = CLFindNode(head,key);
	if(node != NULL)
	{
		nodeData = node->nodeData;
		cout << "key = " << nodeData.key << nodeData.name << nodeData.age << endl;
	}
	else
	{
		cout << "can not find key in this chain list" << endl;
	}
	return 0;
}
